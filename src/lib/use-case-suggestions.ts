...
// ---------------------------------------------------------------------------
// Rules-based startup ↔ use-case (challenge) suggestions for screening.
//
...
// human-readable German reason so partners can trust the "Passende Vorschläge"
// surfaced on the use-case evaluation page.
// ---------------------------------------------------------------------------

interface ChallengeForMatch {
  id: string;
  title: string;
  description: string;
  tags: string[];
...
/** Minimum score for a startup to be suggested. */
const MATCH_THRESHOLD = 18;

/** Max suggestions surfaced per use-case. */
...
/**
 * Scores how well a scouted startup fits an open use-case and produces the
 * "warum das passt" reasons. Returns score 0 when there is no meaningful
 * signal at all.
 */
function scoreStartupForChallenge(
...
/**
 * Ranks candidate startups for a use-case, excluding ids already routed
 * (applied), and returns the top matches above threshold.
 */
