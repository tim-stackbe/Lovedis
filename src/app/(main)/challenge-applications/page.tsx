...
/**
 * Team-wide, READ-ONLY overview of every startup application to every
 * challenge.
 *
 * The per-challenge `Bewerbungen` list on /challenges/[id] only ever answers
 * "who applied to THIS challenge", so scanning across rounds meant opening each
 * challenge in turn. This is the cross-challenge counterpart.
 *
 * Accept/reject is deliberately NOT offered here: the single place to decide an
 * application is the ADMIN-only pitch review on /challenges/[id]. This overview
 * only surfaces status, links and the PoC column so the team can navigate.
 *
...
export default async function ChallengeApplicationsPage({
  ...
  // The overview is team-visible (ADMIN + MEMBER) and read-only for everyone —
  // accepting/rejecting lives solely on the ADMIN-only /challenges/[id] review.
  await requireTeam();
